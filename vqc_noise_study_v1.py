...
MODEL_KIND = "VQC_NOISE"
# ═══════════════════════════════════════════════════════════════════════════
# NOISE STUDY ADDITIONS — everything above is verbatim from v8
# ═══════════════════════════════════════════════════════════════════════════
...
DRM_MAP_NOISE = {
    "PCA4":"pca4","ICA4":"ica4","XGB_PCA4":"xgb_pca4","AUTOENCODER":"autoencoder",
}

# IBM Falcon calibrated parameters
# T1=100μs T2=80μs t_1q=50ns t_2q=300ns readout_error=2%
HIM_PARAMS = dict(
    p_dep_1q  = 0.001,   # single-qubit gate depolarizing error
    gamma_1q  = 0.0005,  # amplitude damping: 1-exp(-50ns/100μs)
    lambda_1q = 0.0006,  # phase damping:     1-exp(-50ns/80μs)
    p_dep_2q  = 0.010,   # CNOT depolarizing (~10x single-qubit)
    gamma_2q  = 0.003,   # amplitude damping: 1-exp(-300ns/100μs)
    lambda_2q = 0.004,   # phase damping:     1-exp(-300ns/80μs)
    p_readout = 0.020,   # 2% classical readout misclassification
)

def build_noise_model(noise_type: str, noise_p: float):
    """
    Build qml.NoiseModel for gate-type-specific noise insertion.

    Insertion rules (based on PDF guide and arXiv references):
      Single-qubit gates (RX/RY/RZ/IsingZZ/IsingXX/CPhase):
        → amplitude_damping, phase_damping, depolarizing at noise_p
      Two-qubit gates (CNOT/CZ):
        → depolarizing at 10×noise_p (CNOT is ~10× noisier)
        → amplitude_damping, phase_damping at same noise_p
      Measurement (expval):
        → BitFlip readout error (HIM only)

    HIM (hardware-inspired mixed) uses fixed IBM Falcon parameters.
    noise_p is IGNORED for HIM.
    """
    import pennylane as qml

    is_1q = qml.noise.op_in([
        qml.RX, qml.RY, qml.RZ, qml.Rot, qml.Hadamard,
        qml.IsingZZ, qml.IsingXX, qml.CPhase,
    ])
    is_2q   = qml.noise.op_in([qml.CNOT, qml.CZ])
    is_meas = qml.noise.meas_eq(qml.expval)

    if noise_type == "amplitude_damping":
        def n1q(op, **kw):
            for w in op.wires: qml.AmplitudeDamping(kw["p"], wires=w)
        def n2q(op, **kw):
            for w in op.wires: qml.AmplitudeDamping(kw["p"], wires=w)
        return qml.NoiseModel({is_1q: n1q, is_2q: n2q}, p=noise_p)

    elif noise_type == "phase_damping":
        def n1q(op, **kw):
            for w in op.wires: qml.PhaseDamping(kw["p"], wires=w)
        def n2q(op, **kw):
            for w in op.wires: qml.PhaseDamping(kw["p"], wires=w)
        return qml.NoiseModel({is_1q: n1q, is_2q: n2q}, p=noise_p)
        ...
        def n1q(op, **kw):
            for w in op.wires: qml.DepolarizingChannel(kw["p1q"], wires=w)
        def n2q(op, **kw):
            for w in op.wires: qml.DepolarizingChannel(kw["p2q"], wires=w)
        return qml.NoiseModel(
            {is_1q: n1q, is_2q: n2q},
            p1q=noise_p, p2q=min(noise_p * 10, 1.0))

    elif noise_type == "him":
        prm = HIM_PARAMS
        def n1q(op, **kw):
            for w in op.wires:
                qml.DepolarizingChannel(kw["p_dep_1q"],  wires=w)
                qml.AmplitudeDamping(   kw["gamma_1q"],  wires=w)
                qml.PhaseDamping(       kw["lambda_1q"], wires=w)
        def n2q(op, **kw):
            for w in op.wires:
                qml.DepolarizingChannel(kw["p_dep_2q"],  wires=w)
                qml.AmplitudeDamping(   kw["gamma_2q"],  wires=w)
                qml.PhaseDamping(       kw["lambda_2q"], wires=w)
        def n_meas(op, **kw):
            qml.BitFlip(kw["p_readout"], wires=op.wires)
        return qml.NoiseModel(
            {is_1q: n1q, is_2q: n2q},
            meas_map={is_meas: n_meas}, **prm)
    else:
        raise ValueError(f"Unknown noise_type: {noise_type}")


def parse_drm_vqc_noise(s: str):
    import re
    m = re.match(r"(.+?)\+VQC\((.+?)\+(.+?)\)$", str(s).strip())
    if not m: return None, None, None
    fs  = DRM_MAP_NOISE.get(m.group(1).strip().upper(), m.group(1).lower())
    enc = RENAME_REV_NOISE.get(m.group(2).strip(), m.group(2).strip())
    ans = RENAME_REV_NOISE.get(m.group(3).strip(), m.group(3).strip())
    return fs, enc, ans
...
def build_model(cfg: dict, dev, diff_method: str):
    ...
    # ── Noise injection (noise_model=None → identical to v8) ────────────────
    _qnode_final = qnode
    _noise_model = cfg.get("_noise_model_obj", None)
    _pl_backend  = cfg.get("pl_backend", "default.mixed")
    if _noise_model is not None and _pl_backend == "default.mixed":
        import pennylane as _qml_n
        _qnode_final = _qml_n.add_noise(noise_model=_noise_model)(qnode)
    ...
    class QModel(nn.Module):
        ...
        def forward(self, x):
            ...
            out = _qnode_final(x.cpu(), self.quantum_weights)
...
def worker_entry(job: dict) -> dict:
    ...
    noise_type  = job.get("noise_type",  "none")
    noise_level = job.get("noise_level",  0.0)
    noise_tag   = (f"{noise_type}_p{noise_level:.4f}"
                   if noise_type not in ("none","him")
                   else ("him_calibrated" if noise_type=="him" else "noiseless"))
    run_key = f"{ansatz_name}__{encoding_name}__{noise_tag}"
    feature_set_label = job.get("feature_set", "?")[:8]
    label = (f"{MODEL_KIND}|GPU{physical_gpu_id}|{feature_set_label}|"
             f"{ansatz_name[:9]}|{encoding_name[:11]}|{noise_tag[:14]}")
    pid = os.getpid()
    ...
    try:
        ...
        # Build noise model inside worker (qml.NoiseModel is not picklable)
        pl_backend = cfg_run.get("pl_backend","default.mixed")
        if noise_type != "none" and pl_backend == "default.mixed":
            _nm = build_noise_model(noise_type,
                                     noise_level if noise_type!="him" else 0.0)
        else:
            _nm = None
        cfg_run["_noise_model_obj"] = _nm
        model = build_model(cfg_run, dev, diff_method)

        # ── CRITICAL: remove non-picklable noise model from cfg_run ──────
        # qml.NoiseModel contains local closures (n1q, n2q) that cannot
        # be pickled by multiprocessing spawn. The noise is already captured
        # inside the QNode closure — it does not need to stay in cfg_run.
        cfg_run.pop("_noise_model_obj", None)
        cfg_run["noise_type"]  = noise_type    # store type as string (picklable)
        cfg_run["noise_level"] = noise_level   # store float (picklable)

        # ── Verify noise is wired ─────────────────────────────────────────
        _noise_active = (_nm is not None and pl_backend == "default.mixed")
        print(f"[PID {pid}] Noise active={_noise_active}  "
              f"type={noise_type}  level={noise_level}  backend={pl_backend}",
              flush=True)
        if not _noise_active and noise_type != "none":
            print(f"[PID {pid}] [WARN] Noise NOT applied — "
                  f"check pl_backend is default.mixed", flush=True)
        ...
        # Use encoding_name with noise_tag appended so each noise level
        # gets a unique filename — prevents overwriting across levels.
        _enc_with_noise = f"{encoding_name}_{noise_tag}"
        metrics = save_roc_npz(job["y_test"], test_prob, threshold, ansatz_name, _enc_with_noise, results_dir)
        ...
        save_job_plots(history, ansatz_name, _enc_with_noise, results_dir, model_name=MODEL_KIND)
        ...
        # ── Append to noise study summary CSV (thread-safe via temp file) ──
        import tempfile, shutil
        summary_csv = os.path.join(
            str(Path(results_dir).parent), "noise_study_summary.csv")
        ns_row = {
            "DRM+VQC": job.get("drm_vqc","?"),
            "Noise_Model":  noise_type,
            "Noise_Level":  noise_level,
            "F1":           round(metrics["f1"],  4),
            "F1_pm":        job.get("f1_pm_baseline","—"),
            "AUC":          round(metrics["auc"], 4),
            "AUC_pm":       "—",
            "TPR":          round(cm_data["TPR"], 4),
            "FPR":          round(cm_data["FPR"], 4),
        }
        try:
            ns_df = pd.read_csv(summary_csv) if os.path.exists(summary_csv) else pd.DataFrame()
            ns_df = pd.concat([ns_df, pd.DataFrame([ns_row])], ignore_index=True)
            tmp = summary_csv + ".tmp"
            ns_df.to_csv(tmp, index=False)
            shutil.move(tmp, summary_csv)
        except Exception as _e:
            print(f"[PID {pid}] [WARN] noise summary append failed: {_e}", flush=True)
...
def make_arg_parser():
    ...
    # ── Noise study args (ADDED vs v8) ────────────────────────────────────
    p.add_argument("--jsc_table",    required=True,
                    help="Path to jsc_score_table_{dataset}.csv")
    p.add_argument("--dataset_name", required=True,
                    help="Dataset name e.g. TON_IOT, UNSW_NB15, NF_UNSW_NB15 "
                         "— used to locate feature columns in the CSV.")
    p.add_argument("--top_n",        type=int, default=5,
                    help="Top-N JSC models to include (default=5)")
    p.add_argument("--noise_type",   required=True,
                    choices=["amplitude_damping","phase_damping",
                              "depolarizing","him"],
                    help="One noise type per run. Results append to noise_study_summary.csv.")
    p.add_argument("--noise_levels", default="0.0,0.001,0.005,0.01,0.05,0.10",
                    help="Comma-separated. Ignored for him (uses IBM Falcon params).")
    # ── Original v8 args continue below ───────────────────────────────────
...
def main():
    ...
    # ── NOISE STUDY: Auto-derive feature_sets from JSC table ──────────────
    # If --feature_sets not explicitly provided, read the DRM folder names
    # directly from the JSC table top-N entries so the user does not need
    # to manually specify which DRM folders to iterate over.
    if args.feature_sets:
        feature_sets = [x.strip() for x in str(args.feature_sets).split(',') if x.strip()]
    else:
        try:
            _jsc_tmp = pd.read_csv(args.jsc_table)
            _rc_tmp  = next((c for c in _jsc_tmp.columns if "jsc_rank" in c.lower()), None)
            _dc_tmp  = next((c for c in _jsc_tmp.columns if "drm" in c.lower() or "vqc" in c.lower()), None)
            _top_tmp = _jsc_tmp.sort_values(_rc_tmp).head(args.top_n)
            _fs_seen = []
            for _, _jr in _top_tmp.iterrows():
                _fs_p, _, _ = parse_drm_vqc_noise(str(_jr[_dc_tmp]))
                if _fs_p and _fs_p not in _fs_seen:
                    _fs_seen.append(_fs_p)
            feature_sets = _fs_seen if _fs_seen else ["pca4", "ica4", "xgb_pca4", "autoencoder"]
            print(f"  [NOISE] feature_sets auto-derived from JSC table: {feature_sets}")
        except Exception as _e:
            print(f"  [WARN] Could not auto-derive feature_sets from JSC table ({_e}). "
                  f"Defaulting to all four DRMs.")
    ...
    for train_size in train_sizes:
        for feature_set in feature_sets:
            ...
            # NOISE STUDY: overwrite CONFIGS line with JSC × noise info
            _jsc_disp = pd.read_csv(args.jsc_table)
            _rc_d = next((c for c in _jsc_disp.columns if "jsc_rank" in c.lower()), None)
            _dc_d = next((c for c in _jsc_disp.columns if "drm" in c.lower() or "vqc" in c.lower()), None)
            _top_d = _jsc_disp.sort_values(_rc_d).head(args.top_n)
            _fs_d  = [r for _, r in _top_d.iterrows()
                       if parse_drm_vqc_noise(str(r[_dc_d]))[0] == feature_set]
            _nl_d  = 1 if args.noise_type == "him" else len(args.noise_levels.split(","))
            _actual_jobs = len(_fs_d) * _nl_d
            print(f"║  {'CONFIGS':12s}: {len(_fs_d)} JSC model(s) for {feature_set} "
                  f"× {_nl_d} noise level(s) = {_actual_jobs} jobs "
                  f"| noise_type={args.noise_type}  ║", flush=True)
            ...
            # ── NOISE STUDY: replace full factorial with JSC top-N ──────────────
            _jsc_df   = pd.read_csv(args.jsc_table)
            _rc       = next((c for c in _jsc_df.columns if "jsc_rank" in c.lower()), None)
            _dc       = next((c for c in _jsc_df.columns if "drm" in c.lower() or "vqc" in c.lower()), None)
            _f1c      = next((c for c in _jsc_df.columns if "f1_pm" in c.lower()), None)
            _top      = _jsc_df.sort_values(_rc).head(args.top_n)
            _nlevels  = ([0.0] if args.noise_type=="him"
                          else [float(x) for x in args.noise_levels.split(",")])

            # Filter JSC models matching this feature_set
            _fs_models = []
            for _, _jr in _top.iterrows():
                _fs_p, _enc_p, _ans_p = parse_drm_vqc_noise(_jr[_dc])
                if _fs_p == feature_set:
                    _fs_models.append({
                        "ansatz": _ans_p, "encoding": _enc_p,
                        "drm_vqc": str(_jr[_dc]),
                        "f1_pm_baseline": str(_jr[_f1c]) if _f1c else "—",
                        "jsc_rank": int(_jr[_rc]),
                    })
            if not _fs_models:
                print(f"  [WARN] No JSC models matched feature_set={feature_set}", flush=True)
                continue

            # ── NOISE STUDY: bypass build_per_gpu_queues ──────────────────
            # build_per_gpu_queues expects UNIQUE (ansatz, encoding) pairs.
            # We have model × noise_level = duplicate pairs for different
            # noise levels → wrong distribution. Build jobs directly instead.
            _rdir = os.path.join(
                args.results_dir,
                f"{feature_set}_{train_size}_{args.test_split}")
            _all_noise_jobs = []
            for _noise_p in _nlevels:
                for _m in _fs_models:
                    _all_noise_jobs.append({
                        "gpu_id":          None,  # assigned below
                        "gpu_slot":        0,
                        "feature_set":     feature_set,
                        "ansatz_name":     _m["ansatz"],
                        "encoding_name":   _m["encoding"],
                        "drm_vqc":         _m["drm_vqc"],
                        "f1_pm_baseline":  _m["f1_pm_baseline"],
                        "noise_type":      args.noise_type,
                        "noise_level":     _noise_p,
                        "results_dir":     _rdir,
                        ...
                        "cfg": {**cfg_base,
                                "encoding_name": _m["encoding"],
                                "ansatz_name":   _m["ansatz"],
                                "_noise_model_obj": None},
                    })
            # Distribute round-robin across GPU slots
            for _ji, _job in enumerate(_all_noise_jobs):
                _job["gpu_id"] = gpu_ids[_ji % len(gpu_ids)]
            # Build queues dict in the format run_scheduler expects
            queues = {}
            for _job in _all_noise_jobs:
                _qkey = f"GPU{_job['gpu_id']}/slot0"
                if _qkey not in queues:
                    queues[_qkey] = []
                queues[_qkey].append(_job)
            combos = [(j["ansatz_name"], j["encoding_name"]) for j in _all_noise_jobs]
            n_workers = min(len(gpu_ids) * jobs_per_gpu, len(_all_noise_jobs))
            print(f"  [NOISE] {len(_all_noise_jobs)} jobs: "
                  f"{len(_fs_models)} models × {len(_nlevels)} noise levels. "
                  f"Distributed across {len(queues)} GPU slots.", flush=True)
            for _qk, _qjobs in queues.items():
                _preview = [(j["ansatz_name"][:10], j["encoding_name"][:12],
                             j["noise_level"]) for j in _qjobs]
                print(f"    {_qk}: {_preview}", flush=True)
